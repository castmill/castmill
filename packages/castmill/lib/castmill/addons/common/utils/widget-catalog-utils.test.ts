...
describe("widget-catalog-utils", () => {
  describe("getTranslatedWidgetName", () => {
    ...
    it("falls back to widget.name when locale is missing or translations are unavailable", () => {
      ...
      expect(getTranslatedWidgetName(widgetWithoutTranslations, "sv")).toBe("Default Name");
      expect(getTranslatedWidgetName(widgetWithTranslations)).toBe("Default Name");
    });
  });
  ...
  describe("getTranslatedWidgetDescription", () => {
    it("returns locale-specific description when available", () => {
      ...
      expect(getTranslatedWidgetDescription(widget, "sv")).toBe("Svensk beskrivning");
    });
    ...
    it("falls back to English description, then widget.description", () => {
      ...
      expect(getTranslatedWidgetDescription(widgetWithEnglish, "de")).toBe("English description");
      expect(getTranslatedWidgetDescription(widgetWithoutDescriptions, "sv")).toBe(undefined);
    });
  });
  ...
  describe("getTranslatedWidgetOption", () => {
    it("returns locale-specific option field when available", () => {
      ...
      expect(getTranslatedWidgetOption(widget, "title", "label", "Fallback", "sv")).toBe("Titel");
    });
    ...
    it("falls back to English option field when locale option is missing", () => {
      ...
      expect(getTranslatedWidgetOption(widget, "title", "placeholder", "Fallback", "de")).toBe(
        "Enter title",
      );
    });
    ...
    it("falls back to provided fallback when locale is missing, translations are missing, or field is missing", () => {
      ...
      expect(getTranslatedWidgetOption(widget, "title", "description", "Fallback", "sv")).toBe(
        "Fallback",
      );
      expect(getTranslatedWidgetOption(widget, "title", "label", "Fallback")).toBe("Fallback");
      expect(getTranslatedWidgetOption(createWidget(), "title", "label", "Fallback", "sv")).toBe(
        "Fallback",
      );
    });
  });
