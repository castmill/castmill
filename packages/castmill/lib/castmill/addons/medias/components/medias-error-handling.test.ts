...
describe("Medias Component Error Handling", () => {
  ...
  describe("confirmRemoveResource", () => {
    it("should show conflict error when removeMedia returns 409", async () => {
      ...
      try {
        await MediasService.removeMedia("http://localhost:4000", "org-123", `${mockResource.id}`);
      } catch (error) {
    ...
    it("should show success toast on successful deletion", async () => {
      ...
      expect(mockToast.success).toHaveBeenCalledWith('Media "Success Media" removed successfully');
  ...
  describe("confirmRemoveMultipleResources", () => {
    ...
    it("should show success when all deletions succeed", async () => {
      ...
      expect(mockToast.success).toHaveBeenCalledWith("3 media(s) removed successfully");
      expect(mockToast.error).not.toHaveBeenCalled();
    });
  });
