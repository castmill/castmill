...
export const Maintainance: Component<{
  ...
  const autoRecoverStatusText = () => {
    ...
    const minutesRemaining = Math.max(1, Math.ceil((date.getTime() - Date.now()) / (60 * 1000)));
  ...
  const setAutorecovery = async (until: string | null) => {
    ...
    try {
      await DevicesService.updateDevice(props.baseUrl, props.organizationId, props.device.id, {
    ...
    <div class="maintainance">
      <div class="maintainance-row autorecovery-row">
        <p class="autorecovery-status">
          {t("devices.maintenance.recoverLostSettingsLabel")}: {autoRecoverStatusText()}
        </p>
            ...
            items={[
              {
                ...
                onClick: () => setAutorecovery(new Date(Date.now() + 60 * 60 * 1000).toISOString()),
              },
              {
                key: "forever",
      ...
      <Show when={!props.device.online}>
        <div>
          <p class="offline-message">{t("devices.maintenance.offlineMessage")}</p>
        </div>
      </Show>
    </div>
  );
