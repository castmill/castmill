  ...
  const validateField = (field: string, value: string) => {
    ...
    if (field === "name") {
      ...
      if (!ratioPattern.test(value)) {
        errorsMap.set(
          field,
          props.t("layouts.invalidAspectRatio") || "Invalid format. Use width:height (e.g., 16:9)",
        );
      } else {
        ...
        if (width <= 0 || height <= 0) {
          errorsMap.set(
            field,
            props.t("layouts.invalidAspectRatio") || "Width and height must be positive numbers",
          );
        } else if (width / height > 10 || height / width > 10) {
          errorsMap.set(
            field,
            props.t("layouts.extremeAspectRatio") || "Aspect ratio is too extreme",
          );
        } else {
          errorsMap.delete(field);
        }
  ...
  const isFormValid = () => {
    ...
    if (aspectRatioPreset() === "custom") {
      ...
      if (!ratioPattern.test(customRatio())) {
        ...
        hasValidRatio = width > 0 && height > 0 && width / height <= 10 && height / width <= 10;
      }
    }
    ...
    <form onSubmit={handleSubmit}>
      <div class="add-layout">
        <FormItem
          ...
          id="name"
          value={name()!}
          placeholder={props.t("layouts.enterLayoutName") || "Enter layout name"}
          autofocus={true}
        ...
        <div class="form-item">
          <Dropdown
            ...
            items={ASPECT_RATIO_OPTIONS.map((preset) => ({
              value: preset.value,
              name: preset.value === "custom" ? props.t(preset.label) || "Custom" : preset.value,
            }))}
        ...
        <Show when={aspectRatioPreset() === "custom"}>
          <FormItem
            label={props.t("layouts.customAspectRatio") || "Custom Aspect Ratio"}
            id="customRatio"
            value={customRatio()}
            placeholder="16:9"
            onInput={(value: string | number | boolean) => {
