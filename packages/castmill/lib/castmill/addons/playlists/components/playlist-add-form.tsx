...
export const PlaylistAddForm: Component<{
  t: TranslateFn;
  onSubmit: (name: string, aspectRatio: AspectRatio, teamId?: number | null) => Promise<void>;
  teamId?: number | null;
}> = (props) => {
  ...
  const validateField = (field: string, value: string) => {
    if (field === "name") {
      ...
      const errorsMap = validateCustomRatioField(field, value, props.t, errors());
      setErrors(errorsMap);
    }
  };
  ...
  const getCurrentAspectRatio = (): AspectRatio => {
    if (aspectRatioPreset() === "custom") {
      ...
      const preset = ASPECT_RATIO_OPTIONS.find((p) => p.value === aspectRatioPreset());
      return {
  ...
  const isFormValid = () => {
    ...
    return hasValidName && hasValidAspectRatio && hasNoErrors && isFormModified();
  };
      ...
      <div class="add-playlist">
        ...
        <Show when={aspectRatioPreset() === "custom"}>
          ...
          <Show when={errors().get("aspectRatio")}>
            <div class="error aspect-ratio-error">{errors().get("aspectRatio")}</div>
          </Show>
        </Show>
