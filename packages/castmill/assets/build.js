...
const componentIcons = glob.sync(
  path.join(__dirname, "..", "lib", "castmill", "addons", "*", "components", "icon.tsx"),
);

componentIcons.forEach((entry) => {
  const componentDirName = path.basename(path.dirname(path.resolve(entry, "..")));
  esbuild
    .build({
      ...baseOpts,
      entryPoints: [entry],
      outfile: path.join(__dirname, `../priv/static/assets/addons/${componentDirName}_icon.js`),
    })
...
async function buildAddons() {
  ...
  const componentEntryPoints = glob.sync(
    path.join(__dirname, "..", "lib", "castmill", "addons", "*", "components", "index.tsx"),
  );
  ...
  componentEntryPoints.forEach(async (entry) => {
    ...
    const opts = {
      ...baseOpts,
      entryPoints: [entry],
      outfile: path.join(__dirname, `../priv/static/assets/addons/${componentDirName}.js`),
    };
