defmodule Castmill.Widgets.IntegrationsTest do
  ...
  describe "network integration credentials" do
    ...
    test "upsert_network_credentials/3 creates new credentials", %{
      ...
      assert {:ok, cred} =
               Integrations.upsert_network_credentials(network.id, integration.id, credentials)
    ...
    test "upsert_network_credentials/3 updates existing credentials", %{
      ...
      assert {:ok, cred1} =
               Integrations.upsert_network_credentials(network.id, integration.id, credentials1)

      # Update
      credentials2 = %{"client_id" => "second-id", "client_secret" => "second-secret"}

      assert {:ok, cred2} =
               Integrations.upsert_network_credentials(network.id, integration.id, credentials2)
    ...
    test "set_network_credentials_enabled/3 toggles enabled status", %{
      ...
      {:ok, updated} =
        Integrations.set_network_credentials_enabled(network.id, integration.id, false)

      assert updated.is_enabled == false

      {:ok, re_enabled} =
        Integrations.set_network_credentials_enabled(network.id, integration.id, true)
    ...
    test "delete_network_credentials/2 returns error if not found", %{
      ...
      assert {:error, :not_found} =
               Integrations.delete_network_credentials(network.id, integration.id)
    end
  ...
  describe "discriminator-based caching" do
    ...
    test "create_integration/1 with discriminator_type widget_option requires discriminator_key",
         %{widget: widget} do
      attrs = %{
        widget_id: widget.id,
    ...
    test "compute_discriminator_id/4 for organization type", %{
      widget: widget,
      organization: organization
    } do
      {:ok, integration} =
      ...
      assert {:ok, discriminator} =
               Integrations.compute_discriminator_id(integration, organization.id)

      assert discriminator == "org:#{organization.id}"
    end

    test "compute_discriminator_id/4 for widget_option type", %{
      widget: widget,
      organization: organization
    } do
      {:ok, integration} =
    ...
    test "compute_discriminator_id/4 for widget_option type missing option returns error", %{
      ...
      assert {:error, {:missing_option, "property_id"}} =
               Integrations.compute_discriminator_id(integration, organization.id, nil, options)
    end

    test "compute_discriminator_id/4 for widget_config type", %{
      widget: widget,
      organization: organization
    } do
      {:ok, integration} =
    ...
    test "compute_discriminator_id/4 for widget_config type requires widget_config_id", %{
      widget: widget,
      organization: organization
    } do
      {:ok, integration} =
      ...
      assert {:error, :widget_config_required} =
               Integrations.compute_discriminator_id(integration, organization.id, nil)
    end
  end
