defmodule CastmillWeb.ResourceControllerTest do
  ...
  describe "list medias" do
    ...
    test "searches medias by name and applies pagination", %{conn: conn, organization: organization} do
      ...
      conn = get(conn, "/api/organizations/#{organization.id}/medias", %{search: "big", page: 1, page_size: 2})
      response = json_response(conn, 200)

      # Assert the first page contains the first 2 "big" media items
      assert %{"data" => [%{"name" => "big media1"}, %{"name" => "big media2"}], "count" => 5} = response

      # Request the second page with the same search term
      conn = get(conn, "/api/organizations/#{organization.id}/medias", %{search: "big", page: 2, page_size: 2})
      response = json_response(conn, 200)

      # Assert the second page contains the next 2 "big" media items
      assert %{"data" => [%{"name" => "big media3"}, %{"name" => "big media4"}], "count" => 5} = response
    end
  end
  ...
  describe "create medias" do
    ...
    test "fails to create a new media when data is incomplete", %{conn: conn, organization: organization} do
  ...
  describe "full media lifecycle" do
    test "creates and retrieves a new media from the list", %{conn: conn, organization: organization} do
