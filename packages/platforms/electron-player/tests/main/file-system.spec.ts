...
describe("File System", () => {
  ...
  it("should list files with details", async () => {
    vi.mocked(readdir).mockResolvedValueOnce(["image.png"] as unknown as Dirent[]);
  ...
  it("should delete all files in the directory", async () => {
    vi.mocked(readdir).mockResolvedValueOnce(["image.png", "image2.png"] as unknown as Dirent[]);
  ...
  describe("Downloading", () => {
        ...
        return {
          on: (event: string, cb: (response) => void) => {
            if (event === "response") {
              ...
              queueMicrotask(() => {
                (responseListeners["data"] || []).forEach((cb) => cb("test data"));
    ...
    it("should return error if data cannot be stored", async () => {
      vi.mocked(writeFile).mockRejectedValueOnce(new Error("Failed to store file"));
