...
describe("Tabs component", () => {
  ...
  it("renders with initial tab active", () => {
    ...
    expect(activeButton).toHaveClass(`${styles["tabButton"]} ${styles["active"]}`);
  ...
  it("changes active tab on button click", async () => {
    ...
    expect(tab2Button).toHaveClass(`${styles["tabButton"]} ${styles["active"]}`);
  ...
  it("renders correctly without initialIndex", () => {
    ...
    expect(firstTabButton).toHaveClass(`${styles["tabButton"]} ${styles["active"]}`);
    expect(screen.getByText("Content 1")).toBeInTheDocument();
  });
});
