...
describe("TagEditor", () => {
  ...
  it("applies disabled class and disables add button when disabled", () => {
    ...
    const addBtn = container.querySelector(".add-tag-button") as HTMLButtonElement;
  ...
  it("shows create option when allowCreate is true and search has no match", async () => {
    const { getByText, getByPlaceholderText } = render(() => (
      <TagEditor
        ...
        onTagRemove={vi.fn()}
        allowCreate={true}
        onCreateTag={vi.fn().mockResolvedValue(createTag({ id: 99, name: "New Tag" }))}
      />
    ));
  ...
  it("closes dropdown on Escape key", async () => {
    ...
    expect(container.querySelector(".tag-editor-dropdown")).not.toBeInTheDocument();
  });
