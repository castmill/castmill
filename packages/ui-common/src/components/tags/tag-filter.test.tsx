...
describe("TagFilter", () => {
  ...
  it("renders with default placeholder", () => {
    const { getByText } = render(() => (
      <TagFilter tags={[tag1, tag2]} selectedTagIds={[]} onTagChange={vi.fn()} />
    ));
  ...
  it("renders with label", () => {
    const { getByText } = render(() => (
      <TagFilter tags={[tag1]} selectedTagIds={[]} onTagChange={vi.fn()} label="Tags" />
    ));
  ...
  it("renders selected tags as chips", () => {
    const { getByText } = render(() => (
      <TagFilter tags={[tag1, tag2, tag3]} selectedTagIds={[1, 2]} onTagChange={vi.fn()} />
    ));
  ...
  it("opens dropdown when clicked", async () => {
    const { getByText, container } = render(() => (
      <TagFilter tags={[tag1, tag2, tag3]} selectedTagIds={[]} onTagChange={vi.fn()} />
    ));
  ...
  it("calls onTagChange when a tag is selected", async () => {
    ...
    const { getByText } = render(() => (
      <TagFilter tags={[tag1, tag2, tag3]} selectedTagIds={[]} onTagChange={onTagChange} />
    ));
  ...
  it("renders checkmark character for selected tags", async () => {
    const { getByText, container } = render(() => (
      <TagFilter tags={[tag1, tag2]} selectedTagIds={[1]} onTagChange={vi.fn()} />
    ));
  ...
  it("applies disabled state", () => {
    const { container } = render(() => (
      <TagFilter tags={[tag1]} selectedTagIds={[]} onTagChange={vi.fn()} disabled={true} />
    ));
  ...
  it("calls onFilterModeChange when mode toggle is clicked", async () => {
    ...
    const allButton = container.querySelectorAll(".msf-mode-btn")[1] as HTMLElement;
