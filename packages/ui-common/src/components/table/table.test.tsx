...
describe("Table Component", () => {
  ...
  it("calls onRowClick when a row is clicked", async () => {
    ...
    render(() => <Table columns={columns} data={data} onRowClick={mockOnRowClick} />);
  ...
  it("does not call onRowClick when checkbox label is clicked", async () => {
    ...
    const label = container.querySelector('label[for^="row-checkbox-"][for$="-1"]');
  ...
  it("does not call onRowClick when action button is clicked", async () => {
    const mockOnRowClick = vi.fn();
    render(() => (
      <Table columns={columns} data={data} actions={actions} onRowClick={mockOnRowClick} />
    ));
