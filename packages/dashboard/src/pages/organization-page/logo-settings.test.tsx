...
vi.mock("@castmill/ui-common", async () => {
  ...
  return {
    ...
    MediaPicker: (props: any) => (
      <div data-testid="media-picker">{props.show && <div>Media Picker Open</div>}</div>
    ),
  };
});
...
describe("LogoSettings", () => {
  ...
  describe("Logo Removal", () => {
    it("should send null when removing logo and show success toast", async () => {
      ...
      render(() => (
        <LogoSettings {...defaultProps} currentLogoMediaId={456} onLogoUpdate={onLogoUpdate} />
      ));
      ...
      await waitFor(() => {
        expect(mockToast.success).toHaveBeenCalledWith("Logo removed successfully");
      });
    ...
    it("should update store cache when removing logo", async () => {
      ...
      await waitFor(() => {
        expect(storeModule.setStore).toHaveBeenCalledWith("organizations", "logos", "org-123", {
          mediaId: null,
          url: null,
          loading: false,
          error: false,
    ...
    it("should disable remove button when disabled prop is true", () => {
      render(() => <LogoSettings {...defaultProps} currentLogoMediaId={456} disabled={true} />);
  ...
  describe("Logo Selection", () => {
    it("should update logo and show success toast on selection", async () => {
      ...
      render(() => <LogoSettings {...defaultProps} onLogoUpdate={onLogoUpdate} />);
