...
describe("OrganizationsInvitationPage", () => {
  ...
  describe("Loading and Preview", () => {
    ...
    it("displays invitation details for new user", async () => {
      const { OrganizationsService } = await import("../../services/organizations.service");
      vi.mocked(OrganizationsService.previewInvitation).mockResolvedValue(mockInvitation);
    ...
    it("displays invitation details for existing user", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Login to accept this invitation/i)).toBeInTheDocument();
    ...
    it("initiates signup with passkey when clicking Sign Up button", async () => {
      ...
      vi.mocked(authFetch)
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ signup_id: "123", challenge: "abc123" }),
        } as Response)
        .mockResolvedValueOnce({
          ok: true,
  ...
  describe("Login Flow", () => {
    it("authenticates existing users with passkey directly", async () => {
      ...
      await waitFor(() => {
        expect(authFetch).toHaveBeenCalledWith("http://localhost:4000/sessions/challenges");
      });
  ...
  describe("Authenticated User Flow", () => {
    it("shows accept button for authenticated user", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Accept/i) || screen.getByRole("button")).toBeInTheDocument();
      });
    });
    ...
    it("calls acceptInvitation when authenticated user clicks accept", async () => {
      ...
      await waitFor(() => {
        expect(OrganizationsService.acceptInvitation).toHaveBeenCalledWith("test-token-123");
      });
  ...
  describe("Email Display", () => {
    it("displays email address in styled box", async () => {
      ...
      const emailBox = screen.getByText("newuser@example.com").closest(".email-box");
