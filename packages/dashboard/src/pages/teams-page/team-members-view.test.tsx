...
vi.mock("@castmill/ui-common", async () => {
  const { onMount } = await vi.importActual<typeof import("solid-js")>("solid-js");
...
describe("TeamMembersView - leave team", () => {
  ...
  const renderComponent = () =>
    render(() => (
      <I18nProvider>
        <TeamMembersView organizationId="org-1" teamId={42} onRemove={vi.fn()} />
      </I18nProvider>
    ));
  ...
  it("allows a member to leave when another admin exists", async () => {
    ...
    await waitFor(() => {
      expect(TeamsService.removeMemberFromTeam).toHaveBeenCalledWith("org-1", 42, "user-1");
    });
