...
const TagsPage: Component = () => {
  ...
  const [showConfirmDelete, setShowConfirmDelete] = createSignal<Tag | null>(null);
  const [showConfirmDeleteGroup, setShowConfirmDeleteGroup] = createSignal<TagGroup | null>(null);
  ...
  const tagsByGroup = (groupId: number) => tags().filter((tag) => tag.tag_group_id === groupId);
  ...
  const openCreateTag = (groupId?: number) => {
    ...
    setTagFormData({
      ...
      color:
        group?.color ||
        (TAG_COLOR_PALETTE[Math.floor(Math.random() * TAG_COLOR_PALETTE.length)] as string),
  ...
  const handleTagSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (editingTag()) {
        const updated = await tagsService.updateTag(orgId, editingTag()!.id, data);
  ...
  const openCreateGroup = () => {
    ...
    setGroupFormData({
      name: "",
      color: TAG_COLOR_PALETTE[Math.floor(Math.random() * TAG_COLOR_PALETTE.length)] as string,
    });
  ...
  const handleGroupSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (editingGroup()) {
        const updated = await tagsService.updateTagGroup(orgId, editingGroup()!.id, data);
        setTagGroups(tagGroups().map((g) => (g.id === updated.id ? updated : g)));
  ...
  return (
    <div class={styles["tags-page"]}>
          ...
          {(group) => (
            ...
            >
              <div class={styles["group-header"]}>
                <h2>
                  {group.name}
                  <span class={styles["group-tag-count"]}>{tagsByGroup(group.id).length}</span>
                </h2>
                <Show when={canManageTags()}>
        ...
        >
          <form onSubmit={handleTagSubmit} class={styles["tag-form"]}>
            <FormItem
              ...
              onInput={(value) => setTagFormData({ ...tagFormData(), name: String(value) })}
            />
                ...
                <Dropdown
                  ...
                  onSelectChange={(value) => {
                    ...
                    setTagFormData({
                      ...
                      ...(group?.color && !editingTag() ? { color: group.color } : {}),
                    });
                  }}
                />
              </div>
      ...
      <Show when={showGroupModal()}>
        <Modal
          title={editingGroup() ? t("tags.groups.editGroup") : t("tags.groups.addGroup")}
          description=""
        ...
        >
          <form onSubmit={handleGroupSubmit} class={styles["tag-form"]}>
            <FormItem
              ...
              onInput={(value) => setGroupFormData({ ...groupFormData(), name: String(value) })}
            />
      ...
      <ConfirmDialog
        ...
        onConfirm={() => showConfirmDelete() && handleDeleteTag(showConfirmDelete()!)}
      />
      ...
      <ConfirmDialog
        ...
        onConfirm={() => showConfirmDeleteGroup() && handleDeleteGroup(showConfirmDeleteGroup()!)}
      />
    </div>
  );
};
