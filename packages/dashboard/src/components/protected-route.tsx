...
import { Component, JSX, Show, onMount, createEffect, createSignal } from "solid-js";
...
const ProtectedRoute: Component<ProtectedRouteProps> = (props: ProtectedRouteProps) => {
  ...
  const [onboardingOrgId, setOnboardingOrgId] = createSignal<string | null>(null);
  ...
  onMount(async () => {
    if (!checkAuth()) {
      ...
      try {
        ...
        const incompleteOrg = organizations.find((org) => org.onboarding_completed === false);
   ...
   */
  const loadAddonTranslations = async (addons: AddOn[], currentLocale: string) => {
    ...
    for (const addon of addons) {
      if (addon.translations_path) {
        try {
          ...
          console.warn(`Failed to load translations for addon ${addon.id}:`, error);
        }
      }
    }
  };
      ...
      setStore("onboarding", {
        ...
        completeStep: async (step: OnboardingStep) => {
          try {
            ...
            if (currentProgress?.completed_steps.includes(step) || currentProgress?.is_completed) {
              return;
            }

            const updatedProgress = await OnboardingService.completeStep(currentUser.id, step);
