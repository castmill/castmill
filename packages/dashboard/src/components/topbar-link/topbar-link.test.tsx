...
vi.mock("@solidjs/router", () => ({
  A: ({ href, children }: { href: string; children: any }) => <a href={href}>{children}</a>,
}));

describe("TopbarLink Component", () => {
  it("renders internal link without external prop", () => {
    render(() => <TopbarLink to="/dashboard" icon={TbHelpCircle} text="Dashboard" />);
  ...
  it("renders external link with external prop", () => {
    render(() => (
      <TopbarLink to="https://docs.castmill.io" icon={TbHelpCircle} text="Help" external={true} />
    ));
