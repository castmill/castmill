...
export class ImageComponent implements TemplateComponent {
  ...
  static fromJSON(json: any): ImageComponent {
    return new ImageComponent(json.name, json.opts, json.style, json.animations, json.filter);
  }
...
export const Image: Component<ImageProps> = (props: ImageProps) => {
  ...
  const backgroundSize = props.opts.autozoom ? "cover" : props.opts.size || "contain";
  ...
  onMount(async () => {
    if (imageRef) {
      ...
      if (hasValidUrl) {
        ...
        let cachedUrl = sourceUrl ? await props.resourceManager.getMedia(sourceUrl) : null;
        if (!cachedUrl) {
          // Log warning but don't crash - use original URL as fallback
          console.warn(`Image ${sourceUrl} not found in cached medias, using original URL`);
        }
  ...
  return <div ref={imageRef} data-component="image" data-name={props.name} style={merged}></div>;
};
