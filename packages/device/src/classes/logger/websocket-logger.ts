...
export class WebSocketLogger implements ILogger {
  ...
  constructor(socket: Socket, deviceId: string) {
    ...
    this.channel.join()
      .receive("ok", () => console.log(`Joined channel successfully: ${channelTopic}`))
      .receive("error", (reasons) => console.error(`Failed to join channel: ${reasons}`))
  ...
  async log(level: LogLevel, message: string): Promise<void> {
    ...
    this.channel.push("log", {
      ...
      .receive("timeout", () => console.error("Connection interruption while pushing the message."));
  }
}
