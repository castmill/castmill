...
describe('BaseMenu component', () => {
  ...
  beforeEach(() => {
    mockEntries = [
      ...
      { name: 'Checkbox Item', id: 'checkbox1', type: 'checkbox', state: false, action: vi.fn() },
      {
        name: 'Submenu Item', id: 'submenu1', type: 'submenu', action: vi.fn(), children: [
          { name: 'Sub Action', id: 'subaction1', type: 'action', action: vi.fn() },
          { name: 'Sub Checkbox', id: 'checkbox2', type: 'checkbox', state: true, action: vi.fn() },
        ]
      }
    ];
  });
  ...
  it('shows menu on click', () => {
    const { queryByRole } = render(() => <BaseMenu header={<div>Menu Header</div>} entries={mockEntries} />);
  ...
  it('renders with provided header and entries', () => {
    const {queryByText} = render(() => <BaseMenu header={<div>Menu Header</div>} entries={mockEntries} />);
  ...
  it('responds to keyboard interactions', () => {
    const { queryByRole, queryByText } = render(() => <BaseMenu header={<div>Menu Header</div>} entries={mockEntries} />);
    const menu = queryByRole('menu');
    ...
    fireEvent.keyDown(window, {key: 'ArrowDown', code: 'ArrowDown', charCode: 40});
    fireEvent.keyDown(window, {key: 'Enter', code: 'Enter', charCode: 13});
